...
= \frac{1}{N} \sum_{i=1}^N \mathcal{CL}_i
\end{equation}
...
$\mathcal{CL}^{h,w}$ represents the cross-entropy loss applied
...
\sum_{h=1}^{H_s} \sum_{w=1}^{W_s} \mathcal{CL}^{h,w}
\end{equation}
...
by Ioffe and Szegedy~\cite{ioffe_batch_2015}. When training a model, the layer
...
\alpha \frac{1}{N} \frac{1}{k} \sum_{l=1}^k \mathcal{CL}_i^{h_l,w_l} \right)
\end{equation}
...
as such a 'background noise instance'. For the region descriptor to be
meaningful, it is thus important that the training images contain only
'meaningful' representations of instances and that they are
...
combination of itself with all the descriptors
