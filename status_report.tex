...
\begin{itemize}
    ...
    \begin{enumerate}
        ...
        not implemented yet
    \end{enumerate}
    \item Transform fine-tuned classification net into FCN, then use this
    to predict region of interest for full sized images:

    feature maps are processed by a convolutionalized FC layer
    this gives a new feature map

    only consider the locations of the highest values in that feature map
    according to the others (the considered locations must have values
    noticeably higher than most others - use percentile or something similar
    to obtain this)

    for each feature map of the convolutional features,
    perform a sum aggregation over the regions defined by the locations
    found previously (this means locations where multiple high values
    overlapped are included multiple times)

    use this value for each filter to obtain the descriptor, then normalize,
    perform Shift + FC and normalize
\end{itemize}
